...
- [ ] `stac_items_paged()` unit tests, offline, with mocks that **pass through and
...
- [ ] Signing-after-paging: mocked `items_fetch` appends an unsigned page-2 feature,
      mocked `items_sign` marks features; assert **every** feature is marked
- [ ] Duplicate-id assertion fires on a hand-built collection with a repeated id
- [ ] `items_matched()` completeness guard: fixture a `doc_items` with
      `numberMatched = 99` and 3 features, assert it aborts; assert it does **not** abort
      when `numberMatched` is absent (both known answers)
- [ ] Wiring test: mock the drift-internal `stac_items_paged()` and
...
- [ ] Pin the stale-`next`-link fact so a future rstac change fails by naming the cause
- [ ] Confirm each test FAILS against current `main` before writing the fix

## Phase 2: Page to exhaustion

- [ ] Add `stac_items_paged()` — `stac_search(limit = 500)` → `get_request()` →
      `items_fetch()` → `items_sign(sign_fn)`, in that order
- [ ] Assert no duplicate item ids after paging. This is the one completeness invariant
...
- [ ] `items_matched()`-conditional completeness guard (fires only when non-NULL)
- [ ] **Strip `rel="next"` from `items$links` before returning.** The fix creates this
...
- [ ] Rewire `dft_stac_fetch()`; keep the `n_items` message and zero-item `stop()`
- [ ] Comment **why** this keeps GET where `dft_stac_cube()` uses POST (the cube needs
...
- [ ] Bump the fetch cache key deliberately (salt), so affected caches rebuild once
- [ ] Re-freeze the legacy-hash test (`tests/testthat/test-dft_stac_fetch.R:83-89`) **with
      a comment recording why it moved** — its own comment demands the break be flagged,
      not silently re-frozen
- [ ] Say so in NEWS: this is a one-time re-fetch of small annual rasters, not the cube's
      multi-hour stream

## Phase 4: Network two-answer test

- [ ] Opt-in `DRIFT_TEST_NETWORK=true`, matching the existing pattern
- [ ] Assert the **identical ordered vector of item ids** across `limit=1` and
...
- [ ] Assert an unpaged `limit=1` gives strictly fewer (the defect and the fix give
      different answers on the same call)
- [ ] Assert signed hrefs on an item from beyond the first page
- [ ] Build the unpaged arm inline in the test — no production `paged=` switch whose only
      caller is a test

## Phase 5: Secondary — expose the cache key

- [ ] `attr(result, "cache_key") <- cache_key` beside `attr(result, "stac_items")`
- [ ] Update `@return` (`R/dft_stac_fetch.R:51-53`), documenting it as **per-call, not
      per-year**, since cache filenames are `<year>_<key>`
- [ ] Test recomputes `stac_cache_key()` with the same inputs and asserts equality —
      pinning the attribute to its definition, not merely to non-NULL

## Phase 6: Docs and release

- [ ] `inst/notes/gdalcubes-pc-gotchas.md`: record that the surviving `next` link makes
      the obvious guard unusable, so it is not re-litigated
- [ ] `NEWS.md` with the measured numbers, the cache-key break, and a note that a larger
